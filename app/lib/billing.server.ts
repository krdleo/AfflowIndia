 ...
 * Billing Utilities
 *
 * Three-tier INR pricing model:
 * - FREE: ₹0, 20 affiliates
 * - STARTER: ₹999/mo (~$12), 200 affiliates
 * - PRO: ₹2,999/mo (~$36), unlimited affiliates
 *
 * Uses Shopify Billing API (appSubscriptionCreate mutation).
 * Bills in USD, displays in INR.
 */
...
export async function createSubscription(
  ...
  if (data?.userErrors?.length > 0) {
    ...
    throw new Error(
      `Failed to create subscription: ${data.userErrors.map((e: { message: string }) => e.message).join(", ")}`
    );
  }
